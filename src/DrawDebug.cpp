...
namespace DrawDebug {
    ...
    public:
        ...
        static void Init(void) {
            if (!s_secondsPerCount) {
                ...
                s_qpcWrapMargin =
                    (uint64_t)(-((int64_t)(countsPerSecond * 60)));  // detect if we've wrapped around by a delta
                                                                     // greater than this - also limits max time
    ...
    // Ideally you would do something a bit more clever, but with what little rendering we do storing a lazy cache like
    // this is fine Depth
    ...
    typedef struct RasterState {
        ...
        RasterState(D3DContext& ctx, RasterStateKey& info) {
    ...
    static bool ReadSwapChain() {
        ...
        __try {
            ...
            if (!SUCCEEDED(data.swapChain->GetDesc(&desc))) return false;
    ...
    void SetBlendState(D3DContext& ctx, bool enable, D3D11_BLEND_OP blendOp = D3D11_BLEND_OP::D3D11_BLEND_OP_ADD,
                       ...
                       D3D11_BLEND srcAlpha = D3D11_BLEND::D3D11_BLEND_ONE,
    ...
    public:
        Shader(const ShaderCreateInfo& createInfo, D3DContext& ctx) noexcept : stage(createInfo.stage), context(ctx) {
            ...
            if (stage == PipelineStage::Vertex) {
                ...
                if (!validProgram) logger::error("SmoothCam: A shader failed to compile.");
            } else {
    ...
    private:
        ...
        bool Compile(const std::string& source, const std::string& entryName, const std::string& version) noexcept {
            ...
            auto versionStr = stage == PipelineStage::Vertex ? std::string("vs_").append(version)
    ...
    public:
        explicit VertexBuffer(const VertexBufferCreateInfo& createInfo, D3DContext& ctx) noexcept
            : context(ctx),
              stride(createInfo.elementSize),
              topology(createInfo.topology),
              vertexCount(createInfo.numElements) {
            CreateBuffer(createInfo.elementSize * createInfo.numElements, createInfo.bufferUsage,
                         createInfo.cpuAccessFlags, createInfo.elementData);
        ...
        ~VertexBuffer() noexcept {
            ...
            if (inputLayout) inputLayout = nullptr;
        }
        ...
        D3D11_MAPPED_SUBRESOURCE& Map(D3D11_MAP mode) noexcept {
            ...
            if (!SUCCEEDED(code)) logger::critical("SmoothCam: Failed to map a D3D vertex buffer.");
            return mappedBuffer;
        }
        // Unmap the buffer
        ...
        void CreateIALayout(const IALayout& layout, const std::shared_ptr<Shader>& vertexProgram) noexcept {
            ...
            const auto layoutCode = context.device->CreateInputLayout(
                layout.data(), layout.size(), vertexProgram->binary->GetBufferPointer(),
                vertexProgram->binary->GetBufferSize(), inputLayout.put());
            if (!SUCCEEDED(layoutCode)) logger::critical("SmoothCam: Failed to create input assembler layout.");
        }
    ...
    private:
        ...
        void CreateBuffer(size_t size, D3D11_USAGE usage, uint32_t cpuAccessFlags,
            ...
            if (!SUCCEEDED(code)) logger::critical("SmoothCam: Failed to create D3D vertex buffer.");
        }
    };
...
PS_OUTPUT main(PS_INPUT input) {
    ...
    typedef struct DrawnPoint {
        ...
        DrawnPoint(Point&& a_point, float& a_duration)
            : point(a_point), duration(a_duration){};
    } DrawnPoint;

    typedef struct Line {
        Point start;
        Point end;
        Line(Point&& a_start, Point&& a_end)
            : start(a_start),
              end(a_end){
    ...
    private:
        ...
        void CreateObjects(D3DContext& ctx) {
            ...
            vbInfo.iaLayout.emplace_back(D3D11_INPUT_ELEMENT_DESC{"POS", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 0,
        ...
        void DrawBatch(uint32_t bufferIndex, LineList::const_iterator& begin, LineList::const_iterator& end) {
            ...
            while (begin != end) {
                ...
                start = (start - 0.5f) * 2.f;

                RE::NiPoint3 a_end{begin->end.pos.x, begin->end.pos.y, begin->end.pos.z};
                glm::vec3 endp{a_end.x, a_end.y, a_end.z};
                RE::NiCamera::WorldPtToScreenPt3((float(*)[4])g_worldToCamMatrix, *g_viewPort, a_end, endp.x, endp.y, endp.z, 1e-5f);
                endp = (endp - 0.5f) * 2.f;

                if (start.z < 0 && endp.z < 0) {
                    begin++;
                    continue;
                }

                if ((start.x < -1 && endp.x < -1) || (start.x > 1 && endp.x > 1) || (start.y < -1 && endp.y < -1) || (start.y > 1 && endp.y > 1)) {
                    begin++;
                    continue;
                }

                glm::vec4 f_start{start.x, start.y, start.z, 1.0f};
                glm::vec4 f_end{endp.x, endp.y, endp.z, 1.0f};
    ...
    public:
        ...
        static void DrawLine(const RE::NiPoint3& a_start, const RE::NiPoint3& a_end,
                                  glm::vec4 a_color) {
            glm::vec3 start{a_start.x, a_start.y, a_start.z};
            glm::vec3 end{a_end.x, a_end.y, a_end.z};

            Line line(Point(start, a_color), Point(end, a_color));

            auto drawHandler = DrawHandler::GetSingleton();

            Locker locker(drawHandler->_lock);

            std::unique_lock lock(drawHandler->renderables.lineSegmentsMutex);
            auto& list = drawHandler->renderables.lineSegments;
        ...
        static void DrawCircle(const RE::NiPoint3& a_base, const RE::NiPoint3& a_X, const RE::NiPoint3& a_Y,
            ...
            for (int i = 0; i < a_numSides; i++) {
                ...
                DrawLine(lastVertex, vertex, a_color);
                lastVertex = vertex;
            }
        }
    ...
    private:
        ...
        struct {
            ...
            static inline std::shared_mutex lineSegmentsMutex;

            void ClearLists() {
                std::unique_lock lock(lineSegmentsMutex);
                lineSegments.clear();
            }

...
void DrawDebug::DrawBoundingBox(
...
bool DrawDebug::PointInBox(const RE::NiPoint3& point, RE::NiPoint3& from, RE::NiPoint3& to, RE::NiPoint3& center, RE::NiPoint3 euler) {
    RE::NiPoint3 local = Rotate(point - center, -euler) + center;

    float minX = std::min(from.x, to.x);
    float maxX = std::max(from.x, to.x);
    float minY = std::min(from.y, to.y);
    float maxY = std::max(from.y, to.y);
    float minZ = std::min(from.z, to.z);
    float maxZ = std::max(from.z, to.z);
